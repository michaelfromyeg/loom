#!/usr/bin/env bash
#
# Loom end-to-end demo: real-world use cases, start to finish.
...
loom() { bun "$REPO/packages/cli/src/index.ts" "$@"; }
WORK="$(mktemp -d)"
...
echo "Author a plugin once (loom.yaml + a SKILL.md), no per-harness JSON:"
loom init code-helper --namespace com.acme >/dev/null
loom validate code-helper
echo
echo "Compile it to all five harnesses (Claude, Codex, Cursor, Copilot, OpenCode):"
loom build code-helper --out out
echo
echo "Install into the harnesses actually on this machine (others are skipped):"
loom install code-helper --scope project --cwd project | grep -E "Installed|skipped|artifacts"
...
loom build "$REPO/fixtures/sample-marketplace" --out market
...
loom publish code-helper | grep -E "valid|scan|badges|passed|BLOCKED"
echo
echo "Sign the compiled artifacts, then verify (tamper-evident):"
loom sign code-helper | sed 's/^/   /'
loom verify code-helper | sed 's/^/   /'
...
if loom verify code-helper >/dev/null 2>&1; then echo "   UNEXPECTED: verify passed"; else echo "   verify correctly FAILED (tamper detected)"; fi

h "Use case 4 -- enterprise managed mode"
echo "An org pins installs to its own namespaces; a foreign namespace is blocked:"
if loom install code-helper --scope project --cwd locked --managed com.enterprise >/dev/null 2>&1; then
...
if command -v claude >/dev/null 2>&1; then
  ...
  loom eval "$REPO/fixtures/sample-plugin" || true
else
