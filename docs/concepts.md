...
So a **Loom plugin : a Claude plugin :: source : a compiled binary for one platform**.
The adapter for each harness is the compiler backend that knows that harness's plugin
format. "Plugin" is already the harnesses' own word for their native installable unit --
