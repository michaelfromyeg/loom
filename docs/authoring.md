...
A plugin is a directory with a `weft.yaml` (or `weft.json5`) and the component files it
...
for editor autocomplete (`pnpm --filter @michaelfromyeg/weft-schema emit-schemas`).
...
weft_min_version: 0.1.0         # optional -- minimum Weft version
components:
...
| Badge      | Earned by                                                                       |
|------------|---------------------------------------------------------------------------------|
| `valid`    | `weft validate` / `weft publish` static checks pass                             |
| `tested`   | eval cases exist and the deterministic tier passes on a real harness            |
| `scanned`  | the security scan finds nothing (`weft publish`)                                |
| `signed`   | `weft sign` produces a valid ed25519 signature over the lockfile artifacts      |
| `verified` | publisher namespace ownership proven (MCP-Registry-style challenge; future)     |

- `weft publish` is the deterministic gate: static `valid` plus a security scan plus
...
- `weft sign` / `weft verify` make the compiled artifacts tamper-evident (the `signed` badge).
- `weft index [--federate]` builds a metadata-only index and can ingest the MCP Registry.
- `weft install --managed <namespaces>` is enterprise managed mode: same mechanism, restricted
  to an allowlist.
