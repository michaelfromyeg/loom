...
- `loom publish` is the deterministic gate: static `valid` + a security scan + trace/output
  evals. It exits non-zero (blocking) if validation fails or any available harness fails its
  evals; UNTESTED harnesses never block. `--snapshot` records baseline scores for the next
  release's differential evals.
