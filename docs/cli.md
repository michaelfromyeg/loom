...
```
Author once, compile to every coding-agent harness. (weft v1.2.0)
...
```
Compile a plugin (or a marketplace of plugins) to harness manifests (weft build v1.2.0)
...
```
Print the full CLI reference (a CLI map), generated from the command tree (weft docs v1.2.0)
...
```
Run a component's evals against the real harnesses (reports UNTESTED honestly) (weft eval v1.2.0)
...
```
Reverse-compile an existing native plugin/marketplace into a Weft plugin (weft import v1.2.0)
...
```
Build a metadata index from plugin dirs (optionally federating the MCP Registry) (weft index v1.2.0)
...
```
Scaffold a new plugin (weft.yaml + a sample skill) (weft init v1.2.0)
...
```
Compile + place a plugin (or a whole marketplace) into harness scopes (weft install v1.2.0)
...
```
Run the deterministic publish gate (static valid + trace/output evals) (weft publish v1.2.0)
...
```
Sign weft.lock's artifact set (ed25519) -> weft.sig + weft.pub (the signed badge) (weft sign v1.2.0)
...
```
Remove what install placed into this project (read from its weft.lock) (weft uninstall v1.2.0)
...
```
Re-resolve refs, recompile, and re-place only artifacts whose hash changed (weft update v1.2.0)
...
```
Statically validate a plugin (the valid badge) (weft validate v1.2.0)
...
```
Verify weft.sig against weft.lock and the on-disk artifacts (weft verify v1.2.0)
